...
import api.helpers.done as done_helper
...
async def mark_task_as_done(task_id: int, db: AsyncSession = Depends(get_db)):
    done = await done_helper.get_done(db, task_id=task_id)
    if done is not None:
        raise HTTPException(status_code=400, detail="Done already exists")

    return await done_helper.create_done(db, task_id)
...
async def unmark_task_as_done(task_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return await done_helper.delete_done(db, original=done)